...
const e1 = Fixtures.get('1.Earthfile');
const e2 = Fixtures.get('2.Earthfile');
const e3 = Fixtures.get('3.Earthfile');

describe('modules/manager/earthfile/extract', () => {
  describe('extractPackageFile()', () => {
    ...
    it('detects targets as different depTypes.', () => {
      const res = extractPackageFile(
      ...
      expect(res).toEqual([
        ...
        },
        {
          ...
          datasource: 'docker',
          depName: 'alpine',
          depType: 'another-target',
          replaceString: 'alpine:latest',
        },
      ]);
      const passed = [
        res?.[2].depType === 'another-target',
        res?.[1].depType === 'some-target',
        res?.[0].depType === 'base',
      ].every(Boolean);
    ...
    it('extracts images on adjacent lines', () => {
      const res = extractPackageFile(e1, '', {})?.deps;
      expect(res).toEqual([
        {
          autoReplaceStringTemplate:
            '{{depName}}{{#if newValue}}:{{newValue}}{{/if}}{{#if newDigest}}@{{newDigest}}{{/if}}',
    ...
    it('extracts images from all sorts of (maybe multiline) FROM statements', () => {
      const res = extractPackageFile(e2, '', {})?.deps;
      expect(res).toEqual([
        {
          autoReplaceStringTemplate:
            '{{depName}}{{#if newValue}}:{{newValue}}{{/if}}{{#if newDigest}}@{{newDigest}}{{/if}}',
    ...
    it('extracts images from multi-line ARG statements', () => {
      const res = extractPackageFile(e3, '', {})?.deps;
      expect(res).toEqual([
        {
          autoReplaceStringTemplate:
            ' ARG \\\n' +
    ...
    it('handles updates of multiple ARG values', () => {
      const res = extractPackageFile(
        '# random comment\n\n' +
          ...
          'ARG DUMMY_PREFIX=\n' +
          'FROM ${DUMMY_PREFIX}${NODE_IMAGE_HOST}${NODE_IMAGE_NAME}:${NODE_IMAGE_TAG}${NODE_IMAGE_HASH}\n',
        '',
        {},
      )?.deps;
      expect(res).toEqual([
